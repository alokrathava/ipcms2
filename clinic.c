...
Full Name  : Alokkumar Girishbhai Rathava
Student ID#: 145877205
Email      : agrathava@myseneca.ca
Section    : NII
...
void displayAllPatients(const struct Patient patient[], int max, int fmt) {
    ...
    int i;
    int patientInfo = FALSE;

    for (i = 0; i < max; i++) {
        if (patient[i].patientNumber != FALSE) {
            patientInfo = TRUE;
            break;
        }
    }
...
void addPatient(struct Patient patient[], int max) {
    int i, nextPostion, spaceAvaliable = FALSE;
    for (i = 0; i < max; i++) {
        if (patient[i].patientNumber == FALSE) {
            spaceAvaliable = TRUE;
            nextPostion = i;
    ...
    if (spaceAvaliable == 0) {
        ...
        patient[nextPostion].patientNumber = nextPatientNumber(patient, max);
        inputPatient(&patient[nextPostion]);
